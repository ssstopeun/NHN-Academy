...
class StatementStudentRepositoryTest {
    ...
    @Test
    @Order(2)
    @DisplayName("findById-student10")
    void findById() {
        Optional<Student> studentOptional = studentRepository.findById("student10");
        log.info("student:{}", studentOptional.get());

        Assertions.assertAll(
                ()->Assertions.assertEquals("student10",studentOptional.get().getId()),
                ()->Assertions.assertEquals("학생10",studentOptional.get().getName()),
    ...
    void update() {
        ...
        Assertions.assertAll(
                ()->Assertions.assertEquals("student8",newStudent.get().getId()),
